...
class Environment:
            ...
            for i in range(self.dims[2]-1):
                for j in range(self.dims[0]*self.dims[1]):        
                    ...
                    for action in self.states[i][j].actions.values():
                        # Path arrows color
                        if (i,j) in markedArrows and action in markedArrows:
                            color = (255,255,0)
                            bwidth = 4
                        else:
    ...
    def drawAnimation(self,moves=None):
        ...
        # To show the end state, add empty move:
        path = moves.copy()
        path.append((0,0))
        ...
        def drawStates(grid):
            ...
            for i in range(self.dims[1]):
                for j in range(self.dims[0]):
                    if (grid[i][j] == "S" and path == None) or (curTime,curPos) == (j,i):
                        pygame.draw.rect(screen, (255,255,0), [j*(W+M)+M,i*(W+M)+M,W,W]) 
                    elif grid[i][j] == "P":
                        pygame.draw.rect(screen, (255,0,0), [j*(W+M)+M,i*(W+M)+M,W,W]) 
                    elif grid[i][j] == "R":
    ...
    def drawFull(self,train=None,args=[]):
        """
        Draws the state-transition diagram
        Could be gigantic, beware!
        If a path is generated (for example greedyPolicy), it will draw
        ...
        heatmap = [[0 for _ in range(self.dims[0])] for _ in range(self.dims[1])]
        ...
        def drawStatesAnimation(grid,state):
            ...
            maxHeat = max([max(row) for row in heatmap])
...
def main():
    ...
    #env.drawAnimation(agent.moves)
    #env.drawModel(agent.moves)
    env.drawFull(train=env.SARSA,
                 args = (0.9,0.9,0.1,1)
                )
